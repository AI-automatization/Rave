...
- **Mas'ul:**
- **Holat:** ❌ Boshlanmagan
- **Sabab:** Hozir `useWatchPartyRoom.ts` syncState kelganda DARHOL play qiladi. `scheduledAt` field bilan barcha peer'lar `setTimeout` orqali ANIQ BIR VAQTDA play qilishi kerak.
- **Qilish kerak:**
  - [ ] `useWatchPartyRoom.ts` → `scheduledAt` tekshirish: `delay = scheduledAt - Date.now()`
  - [ ] `delay > 0` → `setTimeout(() => play(), delay)`
  - [ ] `delay <= 0` → `position + |delay|/1000` seek (o'tib ketgan vaqt kompensatsiya)
  - [ ] PAUSE, SEEK uchun ham xuddi shunday
  - [ ] Heartbeat (VIDEO_HEARTBEAT) da scheduledAt yo'q → eski mantiq
- **Fayllar:** `apps/mobile/src/hooks/useWatchPartyRoom.ts`
- **Bog'liq:** T-S054 birinchi bo'lishi SHART
...
- **Mas'ul:**
- **Holat:** ❌ Boshlanmagan
- **Sabab:** Hozir heartbeat PLAY event sifatida keladi → `seekTo()` → video har 5 sekundda "sakraydi". Heartbeat alohida bo'lgandan keyin faqat drift correction kerak.
- **Qilish kerak:**
  - [ ] `useWatchPartyRoom.ts` → `VIDEO_HEARTBEAT` listener
  - [ ] `expected = ownerPosition + (Date.now() - ownerTimestamp) / 1000`
  - [ ] `drift = myPosition - expected`
  - [ ] Drift > 2.0s → force `seekTo()`
  - [ ] Drift 0.3-2.0s → `playbackRate = drift > 0 ? 0.95 : 1.05` (3s keyin 1.0 qaytarish)
  - [ ] Drift < 0.3s → hech narsa
  - [ ] expo-av: `setRateAsync(rate, shouldCorrectPitch: true)`
- **Fayllar:** `apps/mobile/src/hooks/useWatchPartyRoom.ts`
- **Bog'liq:** T-S056 birinchi bo'lishi SHART
...
- **Mas'ul:**
- **Holat:** ❌ Boshlanmagan
- **Sabab:** WebView da `getPositionMs()` faqat cached qiymat qaytaradi. Owner heartbeat uchun REAL pozitsiya kerak.
- **Qilish kerak:**
  - [ ] `useWebViewPlayer.ts` → har 2 sekundda JS injection:
    `video.currentTime` → `postMessage({ type: 'POSITION_POLL', currentTime })`
  - [ ] `POSITION_POLL` handler: `currentTimeMsRef.current` yangilash
  - [ ] Polling faqat `isPlaying = true` bo'lganda (battery tejash)
