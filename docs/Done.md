...
### F-285 | T-S189 | Mobile: in-app browser — deteksiyasiz "baribir urinish" + verify-on-join

- **Bajaruvchi:** Saidazim (Claude sonnet 5)  **Bajarilgan:** 2026-07-28  **Model:** sonnet
- **O'zgarishlar:** `apps/mobile/src/types/index.ts` (`WatchParty` route'ga `needsVerify?: boolean`), `MediaBottomBar.tsx` (deteksiya topilmagan/bot-himoya holatlarida "Baribir urinish" tugmasi — `TryBtn`), `useMediaDetection.ts` (`importMedia()` endi ixtiyoriy `opts?: {needsVerify}` qabul qiladi, create-navigatsiyaga qo'shadi), `MediaWebViewScreen.tsx` (`onTryCurrentPage` — joriy URL bilan `importMediaRef.current(..., {needsVerify:true})`), `WatchPartyScreen.tsx` (yangi `useEffect` — `needsVerify` bo'lsa, xona yuklangach bir martalik `CHANGE_MEDIA` qayta yuboradi), `translations.ts` (`tryCurrentPage`).
