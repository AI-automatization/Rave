...
Сразу после claim, ДО написания кода:

```bash
.claude/scripts/obsidian-note.sh progress weWatch \
...
## Следующий шаг
[конкретный файл:строка или действие]" \
  Saidazim
```

### При каждом чекпоинте (шаг завершён) — ОБЯЗАТЕЛЬНО

После каждого значимого шага (файл изменён, функция готова, этап пройден):
...
## Следующий шаг
[файл:строка — что конкретно делать]" \
  Saidazim
```
...
```bash
.claude/scripts/obsidian-note.sh clear-progress weWatch \
  "T-XXX — Название задачи" "" Saidazim
```
