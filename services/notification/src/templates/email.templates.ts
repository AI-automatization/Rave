...
// Matches brand style of services/auth/src/utils/emailTemplates.ts — minimalist,
// white background, no colored call-out boxes. The logo is the only branded color.
...
const LOGO_HTML = `<img src="cid:${LOGO_CID}" width="164" height="41" alt="weWatch" style="display:block;border:0;max-width:164px;height:auto;"/>`;
...
<tr><td align="center" style="padding:44px 16px 52px;">
  <table role="presentation" width="520" cellpadding="0" cellspacing="0" style="max-width:520px;width:100%;">
    <tr><td align="center" style="padding-bottom:32px;">${LOGO_HTML}</td></tr>
    <tr>
...
export function reengagementEmail(username: string): string {
  const body = `
    ...
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="margin-bottom:28px;">
      <tr>
        <td style="background-color:${C.soft};border:1px solid ${C.border};border-radius:12px;padding:24px;">
          ...
          <table role="presentation" width="100%" cellpadding="0" cellspacing="0">
            ${['Watch Party — смотри вместе с друзьями', 'Battle Mode — кто посмотрит больше?', '25+ достижений — открой новые', 'Чат в реальном времени'].map(item => `
            <tr>
    ...
    <p style="margin:0;font-size:12px;color:${C.faint};font-family:Arial,sans-serif;line-height:1.6;">
