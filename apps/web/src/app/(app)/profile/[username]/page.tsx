...
async function fetchProfile(username: string): Promise<IUser | null> {
  try {
    const res = await fetch(`${BASE}/users/by-username/${username}`, {
      next: { revalidate: 60 },
    });
...
async function fetchAchievements(userId: string): Promise<IAchievement[]> {
  try {
    const res = await fetch(`${BASE}/users/${userId}/achievements`, {
      next: { revalidate: 300 },
    });
    if (!res.ok) return [];
          ...
          <div className="card-body p-6 flex flex-col sm:flex-row items-start gap-6">
            ...
            <div className="flex-1 space-y-3">
              <div className="flex items-start justify-between gap-2">
                ...
                {/* Do'st qo'shish — client component (auth kerak) */}
                <AddFriendButton userId={user._id} username={user.username} />
              </div>
