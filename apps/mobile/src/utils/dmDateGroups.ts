// WeWatch Mobile — DM chat date grouping (Telegram-style inline "Today"/"Yesterday"/date
// separators + jump-to-date support). Plain functions, no date library — the project
// deliberately keeps zero date deps (see useProfileData.ts's own hand-rolled formatDate).
...
export type DMListItem =
  ...
  | { kind: 'message'; id: string; dateKey: string; label: string; message: IDMMessage };
...
/** Flattens messages into a FlatList-ready list, inserting one date separator per new calendar day. */
export function buildDMList(messages: IDMMessage[], t: Translate): DMListItem[] {
 ...
 * messages). Falls back to the closest earlier day when nothing exists on/after the target —
 * mirrors Telegram's "jump to date" landing on the nearest real message instead of failing.
 */
