...
export function FriendCard({ user, onMessage, onOpenProfile }: Props) {
  ...
  return (
    <li className="flex items-center gap-3 border-b border-[var(--ww-line)] px-4 py-3 transition-colors last:border-0 hover:bg-[var(--ww-surface-1)]">
      <button
        type="button"
        onClick={() => { trackClick('friend_card:open_profile'); onOpenProfile?.(); }}
        disabled={!onOpenProfile}
        className={`flex min-w-0 flex-1 items-center gap-3 text-left ${onOpenProfile ? 'cursor-pointer' : 'cursor-default'}`}
      >
