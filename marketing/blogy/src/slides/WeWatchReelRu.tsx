...
// ─── Phase timing (frames) ────────────────────────────────────
// Aligned to ElevenLabs audio (27.5s):
//   Hook 4s | Problem 6s | Solution 3s | Features 10s | CTA 9s = 32s
// (extra tail gives CTA room to breathe after voice ends)
const FPS = 30;
const P = {
  intro:    [0,         4  * FPS],   // 0–120
  problem:  [4  * FPS,  10 * FPS],   // 120–300
  solution: [10 * FPS,  13 * FPS],   // 300–390
  feats:    [13 * FPS,  23 * FPS],   // 390–690  (10s — all 4 cards + voice)
  cta:      [23 * FPS,  32 * FPS],   // 690–960  (9s — "Скоро. Будь первым. wewatch.uz")
};
const TOTAL_FRAMES = 32 * FPS; // 960
export const REEL_RU_DURATION = TOTAL_FRAMES;
  ...
  return (
    <AbsoluteFill>
      ...
      <AbsoluteFill style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', padding: '0 80px' }}>
        <Tag label="ПРОБЛЕМА" color="#EAB308" v={tagIn} />

        <div style={{ fontSize: 148, fontWeight: 900, lineHeight: 0.97, letterSpacing: '-0.03em' }}>
          <div style={{ color: '#fff',    opacity: w1, transform: `translateY(${interpolate(w1, [0, 1], [26, 0])}px)` }}>Смотришь</div>
          <div style={{ color: '#A78BFA', opacity: w2, transform: `translateY(${interpolate(w2, [0, 1], [26, 0])}px)` }}>фильм</div>
          <div style={{ color: '#fff',    opacity: w3, transform: `translateY(${interpolate(w3, [0, 1], [26, 0])}px)` }}>в одиночку?</div>
        </div>
    ...
    <AbsoluteFill>
      ...
      <AbsoluteFill style={{
        ...
        <div style={{
          ...
          Смотри с друзьями вместе.{'\n'}Одновременно. Откуда угодно.
        </div>
      </AbsoluteFill>
    </AbsoluteFill>
  );
...
const FeaturesPhase: React.FC = () => {
  ...
  const features = [
    { icon: '🎬', title: 'WatchParty',        desc: 'Создай комнату, пригласи друзей',          rgb: '124,58,237',  hi: '167,139,250' },
    { icon: '🔄', title: 'Синхронизация',     desc: 'Смотрите вместе с точностью до кадра',    rgb: '14,165,233',  hi: '56,189,248'  },
    { icon: '💬', title: 'Чат и реакции',     desc: 'Делитесь эмоциями в реальном времени',    rgb: '5,150,105',   hi: '52,211,153'  },
    { icon: '🎥', title: 'Много источников',  desc: 'YouTube, VK, Rutube и другие',             rgb: '217,119,6',   hi: '251,191,36'  },
  ];
    ...
    <AbsoluteFill>
      ...
      <AbsoluteFill style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', padding: '0 72px' }}>
        ...
        <div style={{
          ...
          <span style={{ fontSize: 116, fontWeight: 900, letterSpacing: '-0.03em', color: '#fff' }}>4 </span>
          <span style={{
        ...
        <div style={{ fontSize: 32, color: 'rgba(255,255,255,0.3)', marginBottom: 38 }}>На одной платформе.</div>
    ...
    <AbsoluteFill>
      ...
      <AbsoluteFill style={{
        ...
        <div style={{ textAlign: 'center', marginBottom: 28 }}>
          ...
          <div style={{
            fontSize: 120, fontWeight: 900, lineHeight: 0.96, letterSpacing: '-0.03em', color: '#A78BFA',
...
// ─── ROOT ─────────────────────────────────────────────────────
export const WeWatchReelRu: React.FC = () => {
  const frame = useCurrentFrame();

  const CARD_START = 38;
  const STAGGER    = 20;
  const cardFrames = [0, 1, 2, 3].map((i) => P.feats[0] + CARD_START + i * STAGGER);

  return (
    <AbsoluteFill style={{ fontFamily: 'sans-serif', background: '#03020a', whiteSpace: 'pre-line' }}>
      <Audio src={staticFile('audio/bg.mp3')} volume={0.18} />
